...
//Initialisation du local storage, et recuperation des valeurs de la clé "produit" dans le localStorage
let produitLocalStorage = JSON.parse(localStorage.getItem("produit"));
const positionEmptyCart = document.querySelector("#cart__items");



// Si le panier est vide on affiche un message du panier vide on injectant dans le fichier html le paragraphe
function getCart(){
    if (produitLocalStorage === null || produitLocalStorage == 0) {
        const emptyCart = `<p>Votre panier est vide</p>`;
        positionEmptyCart.innerHTML = emptyCart;
    } else {
        //si le panier n'est pas vide, on boucle avec FOR, et on recupere tout dans nouveau variable "produit"
