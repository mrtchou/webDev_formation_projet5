...
function modificationQuantite() {
    let quantiteModifiee = document.querySelectorAll(".itemQuantity");

    for (let i = 0; i < quantiteModifiee.length; i++) {
        quantiteModifiee[i].addEventListener("change", (event) => {
            event.preventDefault();

            //Selection de l'element à modifier en fonction de son id ET sa couleur
            let quantityModif = produitLocalStorage[i].quantiteProduit;
            let quantiteModifieeValue = quantiteModifiee[i].valueAsNumber;

            const resultFind = produitLocalStorage.find((el) => el.quantiteModifieeValue !== quantityModif);

            resultFind.quantiteProduit = quantiteModifieeValue;
...
function deleteProduct() {
    ...
    for (let i = 0; i < btn_supprimer.length; i++) {
        btn_supprimer[i].addEventListener("click", (event) => {
            ...
            produitLocalStorage = produitLocalStorage.filter(element =>
...
function postForm() {
    ...
    //des quil y a "click" sur cette balise "id order" precedement selectione
    //un array est cree puis recuperation des balises "id" du formulaire 
    btnCommander.addEventListener("click", (e) => {
        ...
        //on parcours la variable qui contient le localStorage avec FOR
        //est cette longueur est inseree dans "array idProducts" precedement cree
